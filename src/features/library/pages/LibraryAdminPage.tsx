...
const LibraryAdminPage = () => {
  ...
  const returnLoan = async (loan: any) => {
    const { error } = await supabase.rpc("return_book", { loan_id_input: loan.id });
    if (error) { toast({ title: "Return failed", description: error.message, variant: "destructive" }); return; }
    toast({ title: "Book returned", description: "The inventory count has been updated." });
    await loadData();
  };
  const deleteResource = async (resource: any) => {
    if (!window.confirm(`Remove ${resource.title} from digital resources?`)) return;
    const { error } = await supabase.from("resources").delete().eq("id", resource.id);
    if (error) { toast({ title: "Resource removal failed", description: error.message, variant: "destructive" }); return; }
    toast({ title: "Resource removed", description: `${resource.title} is no longer available to students.` });
    await loadData();
  };
